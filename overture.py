...
def point_to_bbox(point, dist_meters):
    """
    Convert a center point and radius to a bounding box.
    Returns (min_lon, min_lat, max_lon, max_lat).
    """
    lat, lon = point
    # Approximate degrees per meter at this latitude
    lat_per_meter = 1 / 111320  # ~111.32 km per degree latitude
    lon_per_meter = 1 / (111320 * math.cos(math.radians(lat)))

    delta_lat = dist_meters * lat_per_meter
    delta_lon = dist_meters * lon_per_meter
...
def fetch_buildings(point, dist, silent=False) -> GeoDataFrame | None:
    """
    Fetch building footprints from Overture Maps with height data.
    Uses the overturemaps CLI to download buildings within a bounding box.
    """
    lat, lon = point
    cache_key = f"overture_buildings_{lat}_{lon}_{dist}"
    cache_file = _cache_path(cache_key)
    cached = cache_get(cache_key)
    if cached is not None:
        data = cast(GeoDataFrame, cached)
        count = len(data)
        if not silent:
            print(f"  [CACHE HIT] Overture Buildings: {count:,} footprints")
            print(f"    File: {cache_file}")
        return data

    # Calculate bounding box from center point and radius
    bbox = point_to_bbox(point, dist)
    bbox_str = f"{bbox[0]:.6f},{bbox[1]:.6f},{bbox[2]:.6f},{bbox[3]:.6f}"

    if not silent:
        print(f"  [DOWNLOADING] Buildings from Overture Maps ({dist:.0f}m radius)...")
        print(f"    Bbox: {bbox_str}")
        print(f"    Target: {cache_file}")
        print("  [WAITING] Download from cloud storage may take a while...")

    # Create temp file for download
    temp_file = os.path.join(CACHE_DIR, f"temp_overture_{lat}_{lon}.geojsonseq")
    os.makedirs(CACHE_DIR, exist_ok=True)

    try:
        # Use overturemaps CLI to download buildings
        cmd = [
        ...
        if result.returncode != 0:
            if not silent:
                print(f"  [ERROR] Overture download failed: {result.stderr}")
            return None

        # Parse GeoJSONSeq into GeoDataFrame
        features = []
        ...
        if not features:
            if not silent:
                print("  [NONE] Overture Buildings: no data found")
            return None
        ...
        buildings = gpd.GeoDataFrame(properties_list, geometry=geometries, crs="EPSG:4326")

        count = len(buildings)
        if not silent:
            # Count height coverage
            with_height = buildings['height'].notna().sum() if 'height' in buildings.columns else 0
            with_floors = buildings['num_floors'].notna().sum() if 'num_floors' in buildings.columns else 0
            print(f"  [PARSED] Overture Buildings: {count:,} footprints ready")
            print(f"    Height data: {with_height:,} ({100*with_height/count:.1f}%)")
            print(f"    Floor data: {with_floors:,} ({100*with_floors/count:.1f}%)")
        ...
        if not silent:
            print("  [ERROR] Overture download timed out after 10 minutes")
        return None
    except Exception as e:
        if not silent:
            print(f"  [NONE] Overture Buildings: error ({e})")
        return None
    finally:
...
def extract_building_height(row, default_height=12.0, height_per_level=3.5, max_height=200.0):
    """
    Extract building height from Overture Maps schema.
    Priority: height > num_floors > subtype/class inference > default
    Heights are capped at max_height for visualization.
    """
    height_value = None

    # Try direct height (Overture stores as float in meters)
    ...
    # Try num_floors (Overture's floor count field)
    if height_value is None:
        num_floors = row.get('num_floors')
        if num_floors is not None:
            try:
                height_value = float(num_floors) * height_per_level
            except (ValueError, TypeError):
                pass

    # Default height based on Overture building subtype/class
    if height_value is None:
        subtype = row.get('subtype', '')
        bclass = row.get('class', '')

        # High-rise types
        if bclass in ['skyscraper', 'tower'] or subtype == 'commercial':
            if bclass == 'skyscraper':
                height_value = 120.0
            elif bclass == 'tower':
                height_value = 80.0
            elif bclass in ['office', 'hotel']:
                height_value = 40.0
            else:
                height_value = 25.0
        # Residential types
        elif subtype == 'residential':
            if bclass in ['apartments', 'apartment']:
                height_value = 20.0
            elif bclass in ['house', 'detached', 'semidetached_house']:
                height_value = 8.0
            else:
                height_value = 12.0
        # Industrial/agricultural
        elif subtype in ['industrial', 'agricultural']:
            if bclass == 'warehouse':
                height_value = 15.0
            else:
                height_value = 10.0
        # Civic buildings
            ...
            if bclass in ['cathedral', 'church', 'mosque', 'temple']:
                height_value = 25.0
            elif bclass in ['stadium', 'grandstand']:
                height_value = 30.0
            else:
                height_value = 15.0
        # Small structures
        elif bclass in ['garage', 'shed', 'hut', 'cabin', 'carport']:
            height_value = 4.0
        else:
